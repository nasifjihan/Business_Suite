...
> **Status**: ✅ COMPLETE (Editor-side written 2026-08-27, tsc 0 pass pending continuous user-verified browser gates)

### Tables (schema.prisma L380-750 area)
- **5 NEW enums + 1 extended Role Type**: `EmploymentStatus` (6 tones: ACTIVE/INACTIVE/ON_LEAVE/TERMINATED/SUSPENDED/PROBATION) exactly matches 6-family StatusBadge palette emerald/slate/violet/rose/rose/sky. `EmploymentType` (FULL_TIME/PART_TIME/CONTRACT/INTERN). `AttendanceStatus` (5 real values: PRESENT/LATE/ABSENT/HALF_DAY/LEAVE — no HOLIDAY initially vs earlier draft). `LeaveRequestStatus` (PENDING/APPROVED/REJECTED/CANCELLED). `LeaveAccrualUnit` (DAYS/HOURS). 2 NEW rows upserted to Role enum during seed: HR_ASSISTANT & HR_MANAGER removed — instead existing RoleType `HR` acts as HR power user with all 29 codes.
- **6 NEW tables**: `Department` (code, name unique; managerId→User FK; location; status ACTIVE/INACTIVE enum UserStatus). `Designation` (code, name unique; departmentId FK; isActive Bool NOT status enum — different pattern! Frontend passes `isActive` filter hence coerced boolean validator). `Employee` (employeeCode String @unique counter EMP-XXXX; userId String? @unique bi-directional link to auth User table for self-service login, firstName/lastName/email(unique)/phone/mobile, dateOfBirth @db.Date, joiningDate @db.Date, resignationDate? @db.Date, departmentId, designationId, managerId FK SELF-REFERENCE same Employee model with named relation `@relation("EmployeeManagerLink")` — subordinates[] opposite array required on both sides to prevent Prisma P1012. employmentType enum, status enum EmploymentStatus (NOT named employmentStatus in DB — field name shorter causes front-end/back-end alignment field bugs below!). basicSalary Decimal(14,2)? default(0). address/city/country text. emergencyName/Phone/Relation (contact info not named emergencyContactX in schema — another field name drift). imageUrl? notes? timestamps 12-column index composite). `Attendance` composite PK @@unique([employeeId, attendanceDate]) — one row max per person per day. checkInAt/checkOutAt @db.Time only (not date), workingHours Decimal(4,2), status AttendanceStatus default PRESENT, checkInMethod enum, note?. `LeaveType` code/name unique, defaultDays Int, accrualUnit, carryOverLimit, requiresAttachment, isActive, desc. `LeaveRequest` employeeId/leaveTypeId FKs, startDate/endDate Date, totalDays Decimal(4,2), reason String 2000, status enum default PENDING, approverId Employee self-reference FK, approvedAt, rejectionNote, attachmentUrl?, canceledAt. 9 indexes.
- **+ 6 OPPOSITE-SIDE RELATION arrays mandatory on targets to prevent P1012 Prisma validation "missing opposite relation field" (Phase 7 lesson permanent cure)**. Employee.department → Department.employees[]; Employee.designation → Designation.employees[]; Attendance.employee → Employee.attendances[]; LeaveRequest.leaveType → LeaveType.leaveRequests[]; LeaveRequest.approver → Employee.approvedLeaves[] @relation("LeaveApproverLink") name-matched BOTH sides; Employee.manager(self) → Employee.subordinates[] @relation("EmployeeManagerLink"). If any of these 6 opposite arrays are missing → Prisma generate P1012 fails immediately (user hit 3 such errors Phase 7 on Refund opposite relations).

### 29 Permission Codes Additive seed_phase8_hrm.ts (7 Role tiers)
- **No deleteMany!** `prisma.permission upsert(where code, create, update)` for 29 rows in loop, then role_permissions `createMany skipDuplicates`.
- **SUPER_ADMIN / ADMIN / HR (HR power user)**: all 29 hrm.* codes
- **MANAGER**: hrm.employees.read_team, hrm.attendance.read_team, hrm.leave.read_team + hrm.leave.approve → 9 codes (cannot delete/edit employees, only approve leave on own team filtered by managerId)
- **SALES / CASHIER**: self-service only 6 codes (hrm.departments.read, hrm.designations.read, hrm.employees.read_ME, hrm.attendance.self_check_in, hrm.attendance.read_ME, hrm.leave.create_ME + hrm.leave.read_ME + hrm.leave.cancel_ME — actually 6). Departments & Designations read needed so their GlobalSelect manager/filters work (otherwise 403 on cascading selects for empty forms).
- **VIEWER**: 6 list-read only codes no salary no DOB no emergency (privacy).

### Backend 24 files = 7 resources × 4 layers
- Resources: departments/ (4), designations/ (4), employees/ (4), attendance/ (4 including self check-in/check-out/mark admin), leaveTypes/ (4), leaves/ (4 including approve/reject/cancel actions), reports/ (4: controller summary endpoint 8 aggregations no new tables pure SELECT).
- Master mount: `backend/src/routes/index.ts` line 38-42 area → `apiV1Router.use("/hrm", hrmRouter)` AFTER the sales mount. Aggregator `backend/src/modules/hrm/routes.ts` mounts departmentsRouter, designationsRouter, employeesRouter, attendanceRouter, leaveTypesRouter, leavesRouter, reportsRouter at `/departments`... etc.
- **Key business logic patterns**:
  1. **Attendance composite natural PK upsert**: Check-in endpoint does `prisma.attendance.upsert(where: { employeeId_attendanceDate: { employeeId, date } }, create: {...}, update: { checkInAt, method })` → guarantee never 2 rows same person same day.
  2. **Check-out auto-status override <4 hrs HALF_DAY rule**: If `(checkOutAt-checkInAt) < 4h and status was PRESENT (default admin-not-explicitly-set)` → auto set HALF_DAY sky badge. **BUT if admin explicitly marked ABSENT/LEAVE → never override!** Explicit admin wins.
  3. **Leave overlap validation 4-point formula**: `findFirst({ where: { employeeId, AND: [{ startDate: { lte: req.endDate } }, { endDate: { gte: req.startDate } }] } })` — this catches partial overlaps (Mon-Thu blocks Wed-Fri) as well as identical ranges. If any row returns → 409 Conflict.
  4. **Leave APPROVE 5-step $transaction (Phase 7 multi-step pattern reused)**: (1) SELECT status=PENDING guard else 422 not pending. (2) UPDATE set status=APPROVED, approverId=req.user.employeeId, approvedAt=NOW(). (3) Loop date range from start→end, isWeekday filter, build array of attendance rows. (4) `prisma.attendance.createMany( { data: array, skipDuplicates: true } )` skip if marked holiday or already explicit PRESENT earlier. (5) writeAudit 2 rows bulk inside SAME tx so that rollback on attendance unique-violation → rolls back approval too (Express middleware audit can NOT do this pattern since it fires after commit).
  5. **Self-service security NOT just PermissionGate middleware → real service layer where clause scoping**: attendance.list endpoint `if (!permissionCodes.includes('hrm.attendance.read_all')) where.employeeId = req.user.employeeId;`. This means even if a clever Sales user POSTs Postman to `/hrm/attendance` with param employeeId=Bob (CEO), server still overwrites where clause to return only his rows → security. Front-end PermissionGate buttons are only UX hide; server enforces data visibility.

### Endpoints alignment → many isActive vs status validator bugs:
- 3 GET list endpoints had 422 VALIDATION error because front-end filter queries `?isActive=true` (common UI pattern) vs back-end expecting `status: UserStatus` enum (or for designation no status column only isActive boolean). Fixed by **adding `isActive: z.coerce.boolean().optional()` to list schemas in validators.ts for departments/designations/employees** + service map: `if q.isActive true → where.status=ACTIVE (for department/employee enum) OR where.isActive=true (for designation boolean)`. Avoids 422 when user toggles filters.

### Frontend 6 pages + layout drawer (all "use client")
- **Drawer order final (Phase 7 drawer priority lesson permanently applied)**: Dashboard flat 2 top nav items (Overview, Connectivity) → **Sales & POS drawer topmost business (highest click freq → cashiers see first)** → Inventory → CRM → **HRM drawer (4th, mid freq weekly data entry)** → Administration. HRM drawer 6 links: HRM Overview UserCog, Departments Building2, Designations Briefcase, Employees UserPlus, Attendance Clock, Leaves CalendarRange. Guard `hasAnyHRM = useHasPermission any: [7 codes]` so SALES role sees only drawer (they have self-serve 6 codes, so any-1=true → drawer renders correctly not hidden).
- **Side-effect import `@/lib/api/hrmEndpoints.ts` added to `layout.tsx` (dashboard entry)** — Redux `injectEndpoints` singleton module pattern fails silently if user navigates before first import → hooks not registered. Sales/inventory/crm endpoints already imported there; add hrm one.
- **apiSlice.ts static tagTypes literal array add 7 new: Departments/Designations/Employees/AttendanceRecords/LeaveTypes/Leaves/HRReports** (NEVER runtime push - P6 TS2322 lesson re-confirmed). Widened 6 mutation body types from `Partial<Item>` (too strict, field-name misalignments fail at compile) → `Record<string, any>` at source in `hrmEndpoints.ts` L127, L131, L148, L152, L173, L181.
- **Pages written**:
  1. `/hrm/page.tsx` Overview: 4 DashboardCard KPI (Headcount emerald / On Leave today violet / Attendance % Present sky / Pending Leaves review slate). 6 statuses pure CSS method-like bars width:pct% style inline for Today Attendance breakdown (same Phase7 reports pattern → 0 chart libs). Upcoming work anniversaries (joinDate month matches current month, 8 cards). Recent 5 joiners sorted joiningDate DESC. 2 top action buttons: New Employee, Mark Attendance.
  2. `/hrm/departments/page.tsx` 7 cols code/name/manager/# employees computed client side from employee map (count employees where departmentId)/StatusBadge status ACTIVE/manager select GlobalSelect users list/create-edit GlobalModal Switch isActive.
  3. `/hrm/designations/page.tsx` 6 cols code/name/department/# employees/isActive Toggle/Create GlobalModal Department GlobalSelect filter.
  4. `/hrm/employees/page.tsx` 8 cols employeeCode mono font/avatar name + email/Department badge/Designation name/Manager avatar + initials/StatusBadge employmentStatus 6 families/JoiningDate DateDisplay/actions. **HARD bugs user-verified fixed during gates**: (a) StatusBadge `.charAt(0)` NPE on undefined because Employee response field `status` but frontend reading `employmentStatus` — fix `(e as any).status ?? e.employmentStatus ?? "INACTIVE"` guard with fallback + `typeof string .length` before charAt. (b) Salary field response back-end named `basicSalary Decimal` vs front-end field reading `salary` → form always 0. Fixed both sides: widened body type + fallback `e.basicSalary ?? e.salary`. (c) 3 emergency contact fields response emergencyName vs front-end schema emergencyContactName (extra "Contact" word). Fallback: `emergencyContactName ?? emergencyName`. (d) URL toolbar filters GlobalSelect Status: front-end URL param `employmentStatus=` vs actual back-end list validator field name `status` → no filtering at all. Fix: pushParams `status:` only, read both keys for back-compat but write only correct one. (e) useEffect deps `[modal, form, departments, designations]` → infinite reset every render because array/forms new object ref → fix: only scalar dep array: `[modal.kind, empId, firstDeptIdScalar, firstDesigIdScalar]` (see #8 mistake).
  5. `/hrm/attendance/page.tsx` 7 cols Employee avatar/attendanceDate DateDisplay/checkIn @ Clock icon/checkOut, working hours hrs.min, StatusBadge 5 strict tones/note. Top big buttons: Check In (admin: opens employee select) / Check Out. Self role: buttons disabled for other employees, filter pre-scoped to ME.
  6. `/hrm/leaves/page.tsx` Radix Tabs component: My Requests (everyone, default) / All Requests (HR gate 403 for SALES). Cols: employee avatar/leave type name/date range span/totalDays pill/StatusBadge PENDING/APPROVED/REJECTED/CANCELLED 4 fam/reason 80char trunc/requester/approver. Approve ConfirmDialog destructive emerald. Reject GlobalModal rose required rejectionNote textarea min 20. Cancel own self (only if status PENDING) violet ConfirmDialog. Create leave GlobalModal: LeaveType GlobalSelect, 2 GlobalDatePicker from/to, `calculateWeekdays()` live totalDays below the endDate field (subtract Sat/Sun using date-fns eachDayOfInterval + isWeekend — date-fns already in package.json no new install). reason textarea min 20 chars.

### Common Bugs & fixes during user gates:
1. **Prisma P1012 opposite relations always needed on 1:N or 1:1 self refs — hit 6 times first pass schema.**
2. **Seed validation errors: wrong role.name HR_MANAGER not in RoleType enum → schema only allows SUPER_ADMIN/ADMIN/MANAGER/SALES/CASHIER/HR/VIEWER 7. → removed dynamic role upsert, use existing HR as power user.**
3. **Seed Permission upsert missing `module` required field → schema has column module String not null. Added module: "HRM" to all 29 rows. Also removed bogus columns name/category/resource not in actual Permission DB model — only 6 real cols id/code/module/action/description/createdAt.**
4. **Reports controllers asyncHandler import wrong → controllers had `import { asyncHandler } from "../../../lib/errors"` doesn't exist. Use plain typed `async summary(req: Request, res: Response)` method directly like sales reports — no wrapper needed.**
5. **AttendanceStatus enum size mismatch: services declared const array 6 statuses include HOLIDAY but schema enum only has 5 values PRESENT/LATE/ABSENT/HALF_DAY/LEAVE → TypeScript TS2322 string "HOLIDAY" not type AttendanceStatus. Removed HOLIDAY everywhere, will add as Phase 9 public holiday feature later via separate calendar table.**
6. **Pagination MAX_SIZE = 100 backend enforced → frontend HR pages requested pageSize 10000 (to "get all depts for global select") → 422 validation "pageSize must be <=100". GlobalSelect cascading empty department list during designation create. Fixed all pageSize references across attendance/leaves/departments/designations/employees 5 files → pageSize=100 (plenty for <100 departments/designations/500 employees typical SMB).**
7. **GlobalTable queryResult.data double-unwrap bug (POS Phase 7 twin):** Frontend passed `data: (resp)?.data ?? resp` pre-unwrapped inner `{items,meta}` to GlobalTable helper extractItemsAndMeta, which then applies `inner = result.data; items = inner?.data?.items ?? ...` → double unwrap → undefined → always 0 rows even though Network tab shows valid JSON. Root cause pattern: mixing "raw full response" vs "unwrapped items" prop convention. **Fix: pass FULL RTK response object unchanged.** employees/departments/designations changed → pages now show rows.
8. **React useEffect infinite loop `Maximum update depth exceeded` → every HR page hit at least once.** Pattern: useForm reset inside useEffect with deps containing Arrays or form instance (both new ref every render). Designations page: deps [modal, form, departments] → reset → triggers re-render → new departments array created by RTK → effect runs again → infinite. Leaves page same: [modal, createForm, rejectForm, leaveTypes] array deps. Employees page after fixing earlier had depts/desigs arrays too. **Universal fix: always only scalar values (modal.kind, the edited row's id, `firstDeptId = departments[0]?.id ?? ""` scalar cached outside).** Add `// eslint-disable-next-line react-hooks/exhaustive-deps` comment since we intentionally omit forms/arrays (never depend on them — they are controlled mutable refs anyway not state triggers).
9. **Radix UI `SelectPrimitive.Value asChild` Fragment style Warning: React.Fragment can only have key/ref/children.** During typing in designation create modal dropdown, asChild=true caused Radix to inject computed style prop into its Fragment wrapper around first child span. Browser console spam 50 warnings per open. Fix: **remove asChild, pass className on Value directly.** Radix Select.Value accepts className natively without needing to asChild-wrap a child span.
10. **TypeScript TS2353: "Object literal may only specify known properties — emergencyName does not exist in Partial<EmployeeItem>"** because create/update mutations typed `Partial<EmployeeItem>` strict — but EmployeeItem interface has wrong legacy names (employmentStatus/employmentType/salary/emergencyContactName). Instead of cascading edit to Item interface + all 6 pages column access (20 changes), **surgically widen only the 6 mutation body argument types** in hrmEndpoints.ts to Record<string,any> at source. 6 line fix vs 20+ cascading. If time refactor in Phase 11 hardening.

### Commands user-run (still pending if tsc 0/browser gates in progress):
```powershell
cd "g:\MBW Projects\Other\BS\backend"
npx prisma generate            # Regenerate Prisma client after 6 tables + enum augments
npx prisma migrate dev --name phase8_hrm  # Apply to PostgreSQL running
npx ts-node --transpile-only prisma/seed_phase8_hrm.ts  # 29 codes + 7 tier grants
npx tsc --noEmit ; echo DONE-BE
cd ..\frontend ; npx tsc --noEmit ; echo DONE-FE
# (optional 2-3 min): npm run build — Next.js Turbopack full compile check
# Dev servers restart both tabs (to pick up .ts changes): cd backend npm run dev (:5000), cd frontend npm run dev (:3000)
# BROWSER LOGOUT + RE-LOGIN admin@example.com / Admin@123 (REQUIRED: old token minted pre-seed has 0 hrm codes → drawer invisible until new token has codes embedded in JWT)
```

### 16 Browser-verified gates (user confirms PASS via 2 round error bucketing):
P8-4 Gate 1 → Drawer renders, Overview cards load. P8-5 Gate2 Create Dept Sales code SL manager=ADMIN. P8-6 Gate3 Desig Sales Exec dept Sales. P8-7 Gate4 Create EMP-0001 John Doe status PROBATION=sky badge salary 5000. P8-8 Gate5 Check-in 09:10 LATE violet badge; checkout 18:00 → 8.83 hrs status remains LATE (explicit wins). P8-9 Gate6 Admin Mark John Absent yesterday rose. P8-10 Gate7 Leave create Annual 5 day Mon-Fri. P8-11 Gate8 Overlap → 409 no row. P8-12 Gate9 Approve auto creates 5 attendance weekdays LEAVE teal. P8-13 Gate10 Reject rose. P8-14 Gate11 Self login John → only own attendance/leaves. P8-15 Gate12 VIEWER login: all create buttons hidden salary hidden. P8-16 Gate13 URL back-button preserves filter state 3 pages. P8-17 Gate14 No YELLOW/MUSTARD tones anywhere (grep bg-(amber/yellow/gold/orange/mustard)- 0 hits on all HR pages). P8-18 Gate15 Overview Today CSS bar rows widths render percentages correctly (no rechart). P8-RBAC Gate16 SALES login: only self-serve drawer links visible.

### Git commit command (run only after ALL 16 gates PASS verbally):
```powershell
cd "g:\MBW Projects\Other\BS"
git status
git add -A
git commit -m "Phase 8 HRM: 5 enums + 6 tables (Employee self-ref manager, Attendance composite PK). 29 additive permission codes 7 tiers. 24 backend files 7 resources. Leave APPROVE 5-step $tx auto-sync attendance rows. 6 frontend pages + drawer priority order. Infinite useEffect scalar-only deps fix, GlobalTable double-unwrap POS twin fix, GlobalSelect Fragment asChild fixed. tsc 0 both. 16 browser-verified gates."
```

**Concepts learned**: Self-reference FK both sides + explicit Prisma `@relation("NameMatch")` required on BOTH endpoints; composite natural PK @@unique upsert pattern; salary OMIT from list response payload SELECT column defense-in-depth (not blank UI); server-side scope where clause (not front-end gate); overlapping range 4-point formula; useEffect deps scalar-only rule permanent anti-pattern for arrays/forms; GlobalTable full response vs pre-unwrap convention; widened 6 RTK mutation body types rather than cascade interface rename; coercion boolean `z.coerce.boolean().optional()` for URL `?isActive=true/false` strings; 10 Phase 8 bugs above will be re-used in Phase 9 Finance (similar patterns: GL line self-balancing FK pairs, composite PK journal entries, AP/AR overlap validations).
